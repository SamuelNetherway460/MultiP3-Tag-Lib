...
/**
 * Class which encapsulates all ID3v1 genre data and methods to safely access such data.
 *
...
public class Genres {
    ...
    public static final int ID3V1_GENRES_LOWER_BOUND = 0;
    public static final int ID3V1_GENRES_UPPER_BOUND = 191;
    public static final String[] ID3V1_TAG_GENRES =
    ...
    /**
     * Converts a string genre to the corresponding FileTypes.MP3.ID3.ID3v1 genre index.
     *
     * @param genre The string genre to be converted.
     * @return The index of the FileTypes.MP3.ID3.ID3v1 genre.
     * @throws InvalidGenreException Thrown if the genre is not in the list of FileTypes.MP3.ID3.ID3v1 genres.
     */
    public static int convertStringGenreToIndexID3v1(String genre) throws InvalidGenreException {
        ...
        if (genres.contains(genre.toLowerCase())) {
            int index = genres.indexOf(genre.toLowerCase());
            if (index == -1)
